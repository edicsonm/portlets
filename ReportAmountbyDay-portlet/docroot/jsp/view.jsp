...
<% 
	...
	ArrayList<TransactionVO> listAmountByDay = (ArrayList<TransactionVO>)session.getAttribute("listAmountByDay");
	if(listAmountByDay == null) listAmountByDay = new ArrayList<TransactionVO>();
%>
		...
		<div class="row">
			<liferay-ui:search-container emptyResultsMessage="label.empty" delta="30" iteratorURL="<%=renderURL%>" orderByCol="<%=orderByCol%>" orderByType="<%=orderByType%>">
				<liferay-ui:search-container-results>
					<%
						listAmountByDay = Methods.orderReportAmountByDay(listAmountByDay,orderByCol,orderByType);
						results = ListUtil.subList(listAmountByDay, searchContainer.getStart(), searchContainer.getEnd());
						total = listAmountByDay.size();
				...
				<liferay-ui:search-container-row className="au.com.billingbuddy.vo.objects.TransactionVO" rowVar="posi" indexVar="indice" keyProperty="id" modelVar="transactionVO">
					...
					<liferay-ui:search-container-column-text name="label.amount" property="amountDateReport" value="amountDateReport" orderable="false" orderableProperty="amountDateReport"/>
